...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  const locale = post.slug.fr.current === params.slug ? 'fr' : 'en'
  ...
  const metaDescription = post.seo?.metaDescription ? getLocalizedContent(post.seo.metaDescription, locale) : excerpt
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const structuredData = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://www.sidikoff.com/blog/${params.slug}`,
    },
    articleSection: post.categories?.[0] ? getLocalizedContent(post.categories[0].title, locale) : undefined,
