    ...
    <motion.section
      ...
      <div className='container mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-8 lg:gap-12 items-center relative z-10 py-8 sm:py-12 lg:py-0'>
        ...
        <motion.div
          ...
          variants={itemVariants}>          {/* Badge */}{' '}
          <motion.div
            className='inline-flex items-center gap-2 px-4 py-2 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full text-small font-medium border border-indigo-200 dark:border-indigo-800'
            whileTap={{ scale: 0.95 }}>
            <Sparkles className='w-4 h-4' />
            {t('hero.badge') || 'Agence Web Premium'}
          </motion.div>{' '}          {/* Main Title */}
          <motion.h1
            className='text-display font-heading leading-tight max-w-readable'
          ...
          <motion.div
            className='space-y-3 sm:space-y-4 max-w-readable mx-auto lg:mx-0'
            variants={itemVariants}>
            <p className='text-body-xl text-text-secondary leading-relaxed px-2 sm:px-0'>
              {t('hero.description')}
            </p>
            <p className='text-body text-indigo-600 dark:text-indigo-400 font-medium px-2 sm:px-0'>
              {t('hero.slogan')}
            </p>
          </motion.div>{' '}
          {/* Mobile Logo */}
          ...
          <motion.div
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}>
              {' '}              <div className='relative px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold text-button-lg shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 min-h-[48px]'>
            ...
            <motion.a
              ...
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}>
              <div className='px-6 sm:px-8 py-3 sm:py-4 border-2 border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400 rounded-xl font-semibold text-button-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all duration-300 flex items-center justify-center gap-2 min-h-[48px]'>
                {t('hero.viewWork')}
          ...
          <motion.div
            ...
            {[
              ...
              { number: '24/7', label: t('hero.stat3') || 'Support' },            ].map((stat, index) => (
              <motion.div key={index} className='text-center'>
                <div className='text-stats font-heading text-indigo-600 dark:text-indigo-400'>
                  {stat.number}
                </div>
                <div className='text-micro text-text-secondary leading-tight'>
                  {stat.label}
                </div>
              </motion.div>
            ))}
      ...
      <motion.div
        ...
        animate={{ y: [0, 10, 0] }}
        transition={{ duration: 2, repeat: Infinity }}>        <motion.button
          onClick={() => scrollToSection('services')}
          className='flex flex-col items-center gap-1 sm:gap-2 text-text-secondary hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors p-2 sm:p-3 rounded-full'
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}>
          {' '}
          <span className='text-micro font-medium hidden sm:block'>
