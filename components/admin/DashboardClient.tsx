...
export default function DashboardClient() {
  ...
  const fetchDashboardData = async (showRefreshing = false) => {
    try {
      ...
      const { data: newSubmissionsData, error: newSubmissionsError } = await supabase
        .from('contact_submissions')
        .select('*')
        .gte('created_at', twentyFourHoursAgo.toISOString())

      if (newSubmissionsError) throw newSubmissionsError
      ...
      const { data: paidProjectsData, error: paidError } = await supabase
        .from('projects')
        .select('*')
      ...
      if (newSubmissionsCount > previousNewCount && !hasPlayedNotification.current && !isLoading) {
        ...
        try {
          const audio = new Audio('data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEASQAAAI0CAABGAQAABAABEAUAA...')
          audio.volume = 0.3
  ...
  useEffect(() => {
    ...
    const submissionsSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Real-time update for submissions:', payload)
          fetchDashboardData()
        }
      )
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.error('Failed to subscribe to submissions changes')
    ...
    const projectsSubscription = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Real-time update for projects:', payload)
          fetchDashboardData()
        }
      )
      .subscribe((status) => {
    ...
    const quickInterval = setInterval(() => {
      // Quick check for new submissions every 10 seconds
      fetchDashboardData()
    }, 10000)
    ...
    <div className='space-y-8'>
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center'>
        ...
        <div className='flex items-center space-x-4 mt-4 sm:mt-0'>
          <div className='text-sm text-gray-500 flex items-center space-x-2'>
            ...
            <span>
              {error ? 'Connection error' : 'Live data'} - Last updated: {lastUpdated.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
            </span>
          </div>
          <button
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        <div className='bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow'>
          ...
          <div className='mt-4'>
            <div className='flex items-center text-sm'>
              <span className={`font-medium ${stats.newSubmissions > 0 ? 'text-red-600 animate-pulse' : 'text-orange-600'}`}>
        ...
        <div className='bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow'>
          ...
          <div className='mt-4'>
            <div className='flex items-center text-sm'>
              <span className={`font-medium ${stats.overduePayments > 0 ? 'text-red-600 animate-pulse' : 'text-red-600'}`}>
          ...
          <div className='space-y-4'>
            {recentSubmissions && recentSubmissions.length > 0 ? (
              recentSubmissions.map((submission) => (
                <div key={submission.id} className={`flex items-start p-4 rounded-lg transition-all duration-300 ${
                  submission.status === 'new' ? 'bg-red-50 border border-red-200 animate-pulse' : 'bg-gray-50 hover:bg-gray-100'
                }`}>
                  <div className='flex-shrink-0'>
                    <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white text-sm font-medium ${
                      submission.status === 'new' ? 'bg-gradient-to-r from-red-500 to-red-600' : 'bg-gradient-to-r from-blue-500 to-indigo-600'
                    }`}>
                  ...
                  <div className='ml-3 flex-1 min-w-0'>
                    ...
                    <p className='text-xs text-gray-500 truncate'>{submission.project_type || 'General inquiry'}</p>
