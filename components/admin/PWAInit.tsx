...
export default function PWAInit() {
  ...
  const showInstallBanner = useCallback((prompt: BeforeInstallPromptEvent | null) => {
    // Create install banner if it doesn't exist
    if (document.getElementById('pwa-install-banner')) return

    const banner = document.createElement('div')
    banner.id = 'pwa-install-banner'
    banner.className = 'fixed top-0 left-0 right-0 bg-indigo-600 text-white p-3 text-center text-sm z-50 shadow-lg'
    banner.innerHTML = `
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const isInWebAppiOS = (window.navigator as unknown as Record<string, unknown>).standalone === true
