...
export function SeoAuditTool() {
  ...
  return (
    <section
      id='audit-seo'
      className='relative overflow-hidden py-20 sm:py-24 px-4 sm:px-6 lg:px-8 bg-[#F9F7F7] border-y border-[#112D4E]/8'
      aria-labelledby='audit-seo-title'>
      {/* Subtle Refraction Glows */}
      <div className='pointer-events-none absolute inset-0 overflow-hidden'>
        <div className='absolute -top-32 left-1/2 h-[400px] w-[700px] -translate-x-1/2 rounded-full bg-[#3377FF]/[0.07] blur-[120px]' />
        <div className='absolute bottom-0 right-0 h-[300px] w-[300px] rounded-full bg-[#3377FF]/[0.05] blur-[100px]' />
      </div>
      ...
      <div className='relative z-10 mx-auto max-w-5xl'>
        ...
        <motion.div
          ...
          <div className='mb-4 inline-flex items-center gap-2 rounded-full border border-[#112D4E]/10 bg-white px-4 py-1.5 text-xs font-semibold text-[#112D4E]/80 shadow-2xs'>
          ...
          <h2
            ...
            Auditeur SEO Local <span className='text-[#3377FF]'>Lyon & Villeurbanne</span>
          </h2>
          <p className='mx-auto mt-4 max-w-xl text-base text-[#112D4E]/70 font-normal leading-relaxed'>
            Testez votre site gratuitement en 3 secondes. Découvrez pourquoi vos concurrents locaux
            vous devancent sur Google.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className='relative overflow-hidden rounded-3xl border border-[#112D4E]/10 bg-white p-6 md:p-10 shadow-[0_20px_50px_rgba(17,45,78,0.06)]'>
          {/* Input row */}
          ...
          {error && (
            <p className='relative z-10 mt-2 text-xs font-semibold text-rose-600 px-1'>{error}</p>
          )}
                ...
                <div className='space-y-2.5 rounded-2xl border border-slate-200/60 bg-slate-50/80 p-5 backdrop-blur-xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]'>
                  {AUDIT_STEPS.map((s, i) => (
                    <motion.div
                      ...
                      <span
                        className={i <= step ? 'text-slate-800 font-semibold' : 'text-slate-400'}>
                        {s}
                      </span>
                    </motion.div>
                  ))}
              ...
              <motion.div
                ...
                {results.issues.length > 0 ? (
                  <div className='rounded-2xl border border-amber-300/70 bg-amber-50/80 p-5 backdrop-blur-2xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]'>
                    <p className='mb-3 flex items-center gap-2 text-sm font-bold text-slate-900'>
                      ...
                      {results.issues.length} action{results.issues.length > 1 ? 's' : ''}{' '}
                      prioritaire{results.issues.length > 1 ? 's' : ''} détectée
                      {results.issues.length > 1 ? 's' : ''}
                    </p>
                  ...
                  <div className='rounded-2xl border border-emerald-300/70 bg-emerald-50/80 p-5 backdrop-blur-2xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.9)]'>
                    ...
                    <p className='mt-1 text-xs text-emerald-800/90 font-medium'>
                      Toutes les vérifications automatiques sont au vert. Un accompagnement continu
                      vous garantit de garder votre avance.
                    </p>
                  </div>
                )}
                  ...
                  {!emailSent ? (
                    ...
                    <motion.div
                      ...
                      <p className='text-xs font-medium text-slate-700'>
                        Rapport envoyé à <span className='text-slate-900 font-bold'>{email}</span>.
                ...
                <div className='rounded-3xl bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-700 p-8 text-center text-white border border-white/40 shadow-[inset_0_1.5px_2px_rgba(255,255,255,0.5),0_20px_50px_rgba(0,180,216,0.3)] backdrop-blur-2xl'>
                  ...
                  <h3 className='text-xl font-extrabold text-white md:text-2xl'>
                    ...
                    <span className='text-cyan-200 underline decoration-cyan-300/50 underline-offset-4'>
        ...
        <motion.div
          ...
          {[
            ...
            <span
              key={t}
              className='rounded-full border border-slate-200/80 bg-white/80 px-3.5 py-1.5 backdrop-blur-xl shadow-[inset_0_0.5px_0.5px_rgba(255,255,255,0.9)]'>
              {t}
            </span>
          ))}
        </motion.div>
