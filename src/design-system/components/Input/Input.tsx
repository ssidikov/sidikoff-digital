  ...
  "flex w-full rounded-md border px-3 py-2 text-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        filled: "border-transparent bg-muted focus-visible:bg-background focus-visible:ring-1 focus-visible:ring-ring",
        ghost: "border-transparent bg-transparent focus-visible:bg-muted focus-visible:ring-1 focus-visible:ring-ring",
      },
      size: {
        sm: "h-8 px-2 text-xs",
        default: "h-9 px-3",
...
const Label = React.forwardRef<HTMLLabelElement, LabelProps>(
  ({ className, state, required, ...props }, ref) => (
    <label
      ref={ref}
      className={cn(labelVariants({ state, required }), className)}
      {...props}
    />
  )
)
